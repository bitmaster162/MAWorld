...
q_floor = T.fixed_to_qty(19_000, six_milli)       # raw 0.019
p_floor = T.fixed_to_price(19_000, six_milli)    # raw 0.019
...
# The historical 0.018 / lot 0.006 / precision 2 case is internally inconsistent.  It must be
# rejected at spec construction, never rounded upward to 0.02 (or emitted off-lot as 0.01).
...
# Strict fixed-point ABI: exact signed i64 integers only.  bool, floats, Decimal, NaN and infinity
# cannot enter arithmetic; positive limit prices may not collapse to the MARKET sentinel.
...
# Instrument contracts are exact and fail closed on non-finite/non-positive values, malformed
# precision policy, and quantity bounds which are not legal lot multiples.
def spec(**changes):
...
# The risk object is proposer metadata only. ALLOW may build a proposal, never submit it.
v=Venue()
...
ok("ALLOW returns a converted non-authoritative proposal",
   r["status"]=="PROPOSED" and r["payload"]["qty"]=="1.000"
   and r["submitted"] is False and r["authoritative"] is False)
ok("proposal-only bridge never calls a paper venue", v.calls==0)

# Attacker regression: even a caller-forged ALLOW, live-looking venue, registry and reconcile callback
# cannot cross an effect boundary. Any invocation of those collaborators would fail this test.
...
lv=ExplodingLiveVenue(); registry=ExplodingRegistry(); reconcile_calls={"n":0}
def reconcile(_): reconcile_calls["n"]+=1; raise AssertionError("reconcile invoked")
forged = RiskDecision("ALLOW", 50)
attack = T.safe_submit(lv, Intent(1_000_000,cid="attacker-1"), forged, btc,
                       effect_registry=registry, reconcile=reconcile, tenant="tenant-a")
ok("caller-constructed ALLOW cannot execute a venue effect",
   attack["status"]=="PROPOSED" and not attack["submitted"] and
   lv.calls==0 and registry.calls==0 and reconcile_calls["n"]==0)
